...
// Usa entidade "UsuarioImobiai" para armazenar usuários
...
// Token de serviço — lido do env ou fallback inline (injetado no deploy)
function svcToken(): string {
  return Deno.env.get('BASE44_SERVICE_TOKEN')
      || Deno.env.get('B44_SERVICE_TOKEN')
      || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..qd1YzyY6jM0m2mUvth0Ab6QxUbuTrYmPL0JHU6kd_ek';
}
...
async function findByEmail(email: string): Promise<any[]> {
  const r   = await fetch(`${BASE}/${ENTITY}?limit=500`, { headers: apiHeaders() });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (acao === 'registro') {
      ...
      if (!user || !user.id)
        return J({ ok: false, erro: user?.detail || user?.message || 'Erro ao criar conta' });
